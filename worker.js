 ...
 *  Build: “Lite-R13-toner-hardfilter”
 *
 ...
 *   - cliente(id uuid, nombre, rfc, email, telefono, calle, numero, colonia, ciudad, estado, cp)
...
/* ============================ Cantidades ============================ */
const NUM_WORDS = { 'cero':0,'ninguna':0,'una':1,'uno':1,'un':1,'dos':2,'tres':3,'cuatro':4,'cinco':5,'seis':6,'siete':7,'ocho':8,'nueve':9,'diez':10,'once':11,'doce':12,'docena':12,'media':0.5,'media docena':6 };
...
async function startSalesFromQuery(env, session, toE164, text, ntext, now){
  ...
  if (best) {
    ...
    // SIEMPRE 2 mensajes: ficha + cantidad
...
// Palabras que NO deben pasar si el usuario pidió tóner
const NEG_FOR_TONER = /\b(unidad\s+de\s+revelado|revelad|developer|dev(elop(er)?)?|banda|transfer(encia)?|drum|cilindro|fus(or|er)|belt|kit|maint(enance)?)\b/i;
...
async function findBestProduct(env, queryText) {
  ...
  const wantsToner = /(?:^|\W)toner(?:\W|$)/i.test(queryText) || /\bt[oó]ner\b/i.test(queryText);

  const isBadForToner = (p) => wantsToner ? NEG_FOR_TONER.test(normalizeBase(`${p?.nombre||''} ${p?.sku||''}`)) : false;
  const colorOK  = (p) => productHasColor(p, colorCode);
  const familyOK = (p) => productMatchesFamily(p, queryText);

  const score = (p) => {
    let sc = 0;
    if (wantsToner) { if (/\btoner\b/i.test(p?.nombre || '')) sc += 5; else sc -= 3; }
  ...
  // 2) Consulta con exclusiones cuando piden tóner (filtro duro a nivel SQL)
  try {
    const filters = [
      'select=id,nombre,marca,sku,precio,stock,tipo,compatible',
      'order=stock.desc.nullslast,precio.asc',
      'limit=600'
    ];
    if (wantsToner) {
      filters.push('tipo=eq.toner');
      const deny = ['revel', 'unidad de revelado', 'developer', 'banda', 'transfer', 'drum', 'cilindro', 'fusor', 'belt', 'kit', 'maint'];
      deny.forEach(word => filters.push(`nombre=not.ilike.*${encodeURIComponent(word)}*`));
    }
    const qstr = filters.join('&');
    const r2 = await sbGet(env, 'producto_stock_v', { query: qstr }) || [];
